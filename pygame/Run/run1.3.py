'''
V1.3
    优化功能，使用双声道播放声音，将背景音和碰撞特性声音分别放置在不同声道播放,以便达到同事播放的效果
        -使用Channel类play方法播放Sound对象
...
class MainGame():
    ...
    def startGame(self):
        ...
        music = '素材/奔跑吧兄弟a.mp3'
        # 播放音乐
        pygame.mixer.Channel(1).play(pygame.mixer.Sound(music))
        # 创建图片背景
    ...
    def hitObstacle(self):
        for obstacle in MainGame.obstacleList:
            if pygame.sprite.collide_rect(self, obstacle):
                ...
                music1 = 'D:/pythonProject/pygame/tank/素材/shot.mp3'
                pygame.mixer.Channel(2).play(pygame.mixer.Sound(music1))
                self.hit_count += 1
...
class Music():
    ...
    def playMusic(self, channel_num, loop):
        pygame.mixer.Channel(channel_num).play(loops=loop)
