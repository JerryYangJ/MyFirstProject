...
def index():
    ...
    with open("Web/flask/templates/index/profile.html",'r', encoding='utf-8')as f:
        content = f.read()
        return content


@index_blu.route("/profile")
def profile():
    return "<h1>我是个人主页<h1>"


@index_blu.route("/profile1")
def profile1():
...
# 路由传参
@index_blu.route("/profile2/<user_name>")
...
@index_blu.route("/profile3/<user_name>")
...
# 自定义状态码
@index_blu.route('/demo6')
def demo6():
    return '状态码为 666', 666


# 重定向redirect(需要导入）
@index_blu.route("/profile4")
def profile4():
...
@index_blu.route("/profile8/<user_name>/<profile_info>")
def profile8(user_name, profile_info):
    try:
        with open(f"Web/flask/templates/index/{user_name}.txt", 'r', encoding='utf-8')as f:
