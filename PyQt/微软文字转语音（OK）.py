...
class Ui_Form(object):
    ...
    def do_Button_click(self):
        ...
        if self.tabWidget.currentIndex() == 0:
            ...
            rate = self.horizontalSlider.value()
            if rate == 0:
                COMMAND = f'{highPath[0]}+/dge-tts --text {text} --write-media {save_path} --voice {self.comboBox_2.currentText()}'
                ...
                if rate > 0:
                    ...
                    rate = str(rate) + '%'
                COMMAND = f'edge-tts --text {text} --write-media {save_path}  --voice {self.comboBox_2.currentText()} --rate={rate}'
                subprocess.run(COMMAND, shell = True)
