...
from PyQt5.QtWidgets import QApplication, QWidget, QLabel, QLineEdit, QComboBox, QTextEdit, QDateTimeEdit, QPushButton, QMessageBox, QTableWidget,QTableWidgetItem,QMenu,QAbstractItemView,QStyleFactory
...
class AppWidget(QWidget):
    ...
    def init_ui(self):
        ...
        self.table_infos.setHorizontalHeaderLabels([ '编号', '姓名', '性别', '身份证', '专业', '电话', '部门', '地址', '职务', '出生日期', '备注' ])
        # 禁用双击编辑单元格
        ...
        with open('infos.txt', 'r+', encoding='utf-8') as f:
            ...
            for students_info_line in all_student_info_lines:
                ...
                for i in range(len(all_infos)):
                    ...
                    if person_no == str(temp_student_id): all_infos.remove(all_infos[i])
                    break
        with open('infos.txt', 'w', encoding='utf-8') as f:
            for text in all_infos:
                f.write(text + '\n')
            # 重新加载所有信息 self.reload_all_infos()
    ...
    def add_new_student_info(self):
        ...
        print('note', note)
        # 判断有没有填写的信息，如果有则提示，如果没有则存储到文件 infos = [name, gender, ids, skill, phone, department, addr, post, birthday, note] if '' in infos: QMessageBox.information(self, '提示', '输入的信息不能为空') else: person_no = random.randint(10000, 99999) infos.insert(0, str(person_no)) infos_str = ','.join(infos) with open('infos.txt', 'a+', encoding='utf-8') as f: f.write(infos_str + '\n') f.close()
        # 重新加载所有信息 self.reload_all_infos()
