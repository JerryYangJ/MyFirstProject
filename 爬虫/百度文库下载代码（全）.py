...
# https://wenku.baidu.com/view/9cca4a7e80c758f5f61fb7360b4c2e3f572725e2.html
...
def parse_doc(content):
    url_list = re.findall(r'"pageLoadUrl":"(https.*?0.json.*?)"', content)
    for url in url_list:
        print(url)
...
def parse_ppt2(content, title):
    url_list = re.findall(r'(https://wkretype.bdimg.com.*?)"', content)
    url_list = [addr.replace('\\\\/', '/') for addr in url_list]
