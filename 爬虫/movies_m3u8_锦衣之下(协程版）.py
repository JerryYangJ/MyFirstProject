...
@desc: 注意：
        ...
        尝试解决方案：
            ...
            2.用协程池限制最大并发数：semaphore = asyncio.Semaphore(5)  # 设置最大并发数为5

        报错四：
            aiohttp.ClientTimeout
        尝试解决方案：ok
            1.设置最超时时间：timeout = aiohttp.ClientTimeout(total=900)
            2.通过try-except增加报错后重新请求：
...
async def main():
    ...
    # 读取m3u8文件路径
    m3u8_file_path = 'D:\下载\index (25).m3u8'
    # 保存路径
    save_path = './movie/锦衣之下(2)/'
